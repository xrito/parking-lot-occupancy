...
class ParkingService
{
    ...
    public function getSpotAvailable(): array
    {
        ...
        $spotsAvailable = $this->foundCars($snapshotDest, $this->publicPath . '/new.jpg');

        $freeSpotNumbers = array_keys(array_filter($spotsAvailable, fn($spot) => $spot === true));
        ...
        string $dest): array
    {
        ...
        $foundSlots = $this->foundSlots($carPredictions);
        return $foundSlots;
    }
    ...
    private function foundSlots(array $predictions)
    {
        ...
        return array_map(fn($ind) => isset($sectionPredictions[$ind]) && $sectionPredictions[$ind] > 50 ? false : true,
            array_keys($sections));
    }
